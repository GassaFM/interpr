...
* `for <name> := <start> until/downto <finish>:` &mdash; начало for-блока.
За этой строкой следуют одна или несколько команд
с одинаковым более глубоким отступом.
Сначала эта команда присваивает значение выражения `<start>`
переменной с именем `<name>`.
Затем, если использовано ключевое слово `until`, то пока `<name>` строго меньше значения выражения `<finish>`,
команды выполняются сверху вниз, значение переменной увеличивается на единицу,
после чего условие проверяется снова. Если же было использовано ключевое слово `downto`, то пока `<name>` нестрого больше значения выражения `<finish>`,
команды выполняются сверху вниз, значение переменной уменьшается на единицу,после чего условие проверяется снова.
