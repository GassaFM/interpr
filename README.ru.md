...
Ветку `else:`, состоящую из 1 `if <cond>: ...`, можно заменить на `elif <cond>: ...`,
чтобы избежать увеличения отступов ([пример использования](#использование-elif)).
...
### Использование `elif`
2 цепочки `if`-ов ниже делают одно и то же:
```
a := 1
