...
Далее может следовать `else` или `elif` с таким же отступом, как у `if`.
Он будет выполнен, если значение выражения `<cond>` равно нулю.

* `elif <cond>:` &mdash; начало elif-блока.
То же самое, что `else:<перевод строки>if <cond>:`, 
но может следовать только за блоком `if` или `elif`
и требует увеличения отступа 1 раз и занимает 1 строку
вместо 2 увеличений и строк для `else if`.

* `else:` &mdash; начало else-блока.
Может следовать только за блоком `if` или `elif`.
За этой строкой следуют одна или несколько команд
с одинаковым более глубоким отступом.
Эти команды выполняются, 
если значение всех выражений `<cond>` в предыдущих `if` и `elif` было равно нулю.
...
```
function sum (id, pr, n, a):
    # Только один процесс будет что-то считать.
    if id == 0:
        s := 0
        for i := 0 until n:
            s += a[i]
